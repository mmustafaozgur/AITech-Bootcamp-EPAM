...
**Testing Principles (per Constitution v2.1.0):**
- All features MUST be developed using Test-Driven Development (TDD) with tests written before implementation.
- Tests MUST be generated from specifications and acceptance criteria, not implementation details.
- The Testing Pyramid applies: ~70% unit, ~20% integration, ~10% E2E, with ≥80% line, ≥75% branch, and ≥75% mutation coverage required for business logic.
- Test files MUST follow project conventions: `tests/unit/**/*.test.ts`, `tests/integration/**/*.test.ts`, `tests/e2e/**/*.spec.ts`.
- Naming: `ComponentName.test.ts` for unit/integration, `user-journey-name.spec.ts` for E2E; suites use `describe`, cases use `it('should ...')`.
- Arrange-Act-Assert (AAA) pattern is required; use `beforeEach` for setup, no shared global state.
- Mock external services, stub time-dependent functions, use fakes for DB, and extract helpers for test data.
- All tests MUST be fast, deterministic, and test observable behavior only; avoid anti-patterns (see constitution).
- Tools: TypeScript strict mode, ESLint, Vitest/Jest, Playwright/Cypress, Stryker for mutation, pnpm scripts for execution.
...
### Testing Requirements (per Constitution v2.1.0)
- All new business logic MUST be covered by unit tests written before implementation (TDD).
- Integration tests MUST cover all API endpoints and database operations.
- E2E tests MUST cover all critical user workflows (registration, login, password reset, session refresh).
- All test files MUST be named and organized per project conventions.
- Static analysis (TypeScript strict mode, ESLint) MUST pass on every PR.
- Coverage targets: ≥80% line, ≥75% branch, ≥75% mutation score (Stryker).
- All tests MUST be fast (<1s unit, <5s integration), deterministic, and test observable behavior.
- Mocking, stubbing, and fakes MUST be used as appropriate; do NOT mock code you own.
- All test and coverage reports MUST be visible in CI/CD; pre-commit hooks MUST run typecheck, lint, and unit tests.
...
### Testing Success Criteria (per Constitution v2.1.0)
- All business-logic modules achieve ≥80% line and ≥75% branch test coverage, and ≥75% mutation score.
- All user stories have independently testable acceptance scenarios, covered by automated tests.
- All tests are written before implementation and follow AAA pattern, naming, and organization conventions.
- All static analysis, lint, and test checks pass in CI/CD and pre-commit hooks.
